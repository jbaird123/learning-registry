...
public final class SchemaValidation {
	private static JsonNode schemaJson; 
	private static JsonSchemaFactory factory;
	private static JsonSchema schema;
	
	
	
	public static SchemaValidationStatus ValidateSchema(String jsonString) throws IOException, ProcessingException {
		SchemaValidationStatus schemaValidationStatus = new SchemaValidationStatus();
		if (schemaJson==null) {
			schemaJson= JsonLoader.fromString((getLearningRegistryResourceV2Schema()));
	     ...
	     report = (ListProcessingReport) schema.validate(json,true);
	     
	     schemaValidationStatus.setSuccess(report.isSuccess());
	     schemaValidationStatus.setErrorMessageJson(report.asJson().toString());
	     
		return schemaValidationStatus;
	}

	private static String getLearningRegistryResourceV2Schema() {
		/*String schema = ""
				+ "{"
				...
				+ "               \"type\":\"string\","
				+ "               \"pattern\":\"^(\\\\d{1,2})(-|-\\\\d{1,2})?$\","
				+ "               \"description\":\"The typical range of ages for the contentâ€™s intended end user. e.g: '7-9', '18-'\","
				...
				+ "               \"type\":\"string\","
				+ "               \"format\":\"uri\","
				+ "               \"description\":\"A URL that identifies the conditions that govern the userâ€™s ability to access a learning resource, for example: 'https://ceds.ed.gov/element/001561#FreeAccess'. See more on: https://ceds.ed.gov/element/001561\""
				+ "            },"
				+ "            \"author\":{"
				+ "               \"type\":\"object\","
				...
				+ "            },"
				+ "            \"registry_metadata\":{"
				+ "               \"$ref\":\"#/definitions/registry_metadata\""
				+ "            }"
				+ "         },"
				+ "         \"required\":["
				+ "            \"name\","
		...
		String schema = "{"
				...
				+ "            },"
				+ "            \"original_envelope\":{"
				+ "               \"description\":\"JSON string with the original LR 1.0 envelope, if available\","
				+ "               \"type\":\"string\""
				+ "            }"
				+ "         },"
				+ "         \"required\":["
